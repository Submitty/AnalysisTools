...
public:   
  ...
  void print();
  size_type getAlloc() const { return m_alloc; }
...
template<class T> void Vec<T>::print()
{
    ...
    cout << "allocated size: " << m_alloc << endl;
    cout << "acutal size: " << m_size << endl;
...
template <class T> typename Vec<T>::size_type remove_matching_elements(Vec<T>& v1, const T& element)
{
    ...
    if(v1.size() < (v1.getAlloc()/2))
    {
        Vec<T> tempVec;
        for(int i=0; i<tempVec.size(); i++)
        {
            tempVec.push_back(v1[i]);
        }
    }
    return count;
}
