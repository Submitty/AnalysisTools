...
class ds_hashset {
    ...
    public:
        ...
        std::pair< iterator, bool > insert(KeyType const& key) {
            ...
            int hash_value = m_hash(key);
            int index = hash_value% m_table.size();
            m_table[index].push_front(key);
            hash_list_itr p2 = std::find(m_table[index].begin(), m_table[index].end(), key);
            pair<iterator, bool> p(iterator(this, index, p2) , true); 
            return p;
        }
    ...
    private:
        ...
        void resize_table(unsigned int new_size) {
            ...
            list<KeyType> l;
            vector< list<KeyType> > newTable(new_size, l); 
