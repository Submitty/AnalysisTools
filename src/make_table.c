...
static void write_diff(const char *path, unsigned int count, unsigned int *a,
		       unsigned int *b)
{
	FILE *f = fopen(path, "w");
	unsigned int i;
...
static unsigned int parse_pairs(const char *pairs, unsigned int *a,
				unsigned int *b)
{
...
int main(int argc, char **argv)
{
	...
	while (fgets(line, 4096, stdin) != NULL) {
		...
		printf
		    ("<tr><td>%s</td><td><a href=html_summary/%u.html>view</a></td><td>%s</td><td>%s</td></tr>\n",
		     percent_match, pair_id(first, second), first, second);
		count = parse_pairs(pairs, a, b);
		count = smudge_pairs(a2, b2, a, b, count);
		snprintf(buf, STRING_LENGTH, "html_summary/%u.json",
			 pair_id(first, second));
