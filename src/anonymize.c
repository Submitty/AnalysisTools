 ...
 */
/*@only@*/ /*@null@*/ static regexp_entry *REGEXPS = NULL;
...
int main(int argc, char **argv)
{
	...
	while ((delim = next_word(word, STRING_LENGTH)) != (char) 0x0) {
		...
		for (r = REGEXPS; r != NULL; r = r->next) {
			int rc;
			while ((rc = pcre_exec(r->re, NULL, word, (int) strlen(word), 0, 0, ovector, 30)) > 0) {
				unsigned int sublen = (unsigned int) strlen(r->sub);

				memset(buf, 0, STRING_LENGTH);
				memcpy(buf, word, (size_t) ovector[0]);
				memcpy(buf + ovector[0], r->sub, (size_t) sublen);
				memcpy(buf + ovector[0] + sublen, word + ovector[1], (size_t) (STRING_LENGTH - 1 - (ovector[0] + sublen)));
				memcpy(word, buf, STRING_LENGTH);
			}
		}
